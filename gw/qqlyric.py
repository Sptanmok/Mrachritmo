...
def qrc_decode(data):
    ...
    p=subprocess.Popen(['wine','lib_qrc_decoder.exe'],stdin=subprocess.PIPE,stdout=subprocess.PIPE)
    stdout,stderr=p.communicate(data+b'\n\n')
    if stderr:
        raise RuntimeError(stderr.decode(errors='ignore'))
    lines = stdout.splitlines()
    filtered_lines = [line for line in lines if b'0048' not in line]
    if not filtered_lines:
        filtered_lines = lines

    filtered_stdout = b'\n'.join(filtered_lines)
    data=binascii.unhexlify(filtered_stdout.strip())
    try:
